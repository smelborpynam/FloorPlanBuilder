...
(function (FP) {
  ...
  var BODY = [
    ...
    '<h3>Why does moving one wall change other rooms?</h3>',
    '<p>Because a wall is a <b>cut across the plan</b>, not a loose panel. The house is built by slicing it into pieces, and a wall is one of those slices. The short piece you can see between two rooms is often part of a much longer cut, and moving it moves the whole cut &mdash; so every room along that line changes with it.</p>',
    '<p>When you point at a wall, the <b>pale blue line</b> shows the full cut that will move. The solid part is the piece between the two rooms you are pointing at. If the pale line runs the height of the house, so will the change.</p>',
    '<p>Walls you draw yourself with <b>Add Wall</b> are different: they only cut the one room you drew them in, so dragging one only ever changes those two rooms. If you want a wall that moves on its own, draw it rather than reusing a long one from the generated layout.</p>',
